...
function renderComboTable() {
    ...
    if (comboData.allCombos.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding:20px;">Chưa có combo nào</td></tr>';
        return;
    }

    tbody.innerHTML = comboData.allCombos.map(combo => {
        const dateCreated = new Date(combo.createdAt).toLocaleDateString('vi-VN');
        const itemCount = combo.items ? combo.items.length : 0;
        const status = combo.isActive ? '<span style="color:green;">✓ Hoạt động</span>' : '<span style="color:red;">✗ Tắt</span>';
...
function renderProductOptions() {
    ...
    list.innerHTML = comboData.allProducts.map(product => {
        const isSelected = comboData.selectedProducts.some(p => p.productId === product.productId);
        return `
...
function editCombo(comboId) {
    ...
    console.log('DEBUG editCombo - selectedProducts:', comboData.selectedProducts);
    ...
    if (combo.imagePath) {
        document.getElementById('imagePreview').src = `/SELLING-GLASSES/public/assets/images/products/${combo.imagePath}`;
...
function toggleComboProduct(productId, isChecked) {
    if (isChecked) {
        ...
        comboData.selectedProducts = comboData.selectedProducts.filter(p => p.productId !== productId);
    }
}
...
function saveCombo() {
    ...
    .then(data => {
        ...
        if (data.success) {
            showComboAlert(data.message || (isEdit ? 'Cập nhật thành công' : 'Tạo thành công'), 'success');
            closeModal();
            loadAllCombos();
        } else {
...
function searchCombos() {
    ...
    fetch(`${COMBO_API_BASE}/index.php?url=search-combos&name=${encodeURIComponent(searchTerm)}`)
        .then(res => {
            if (!res.ok) {
                throw new Error('Server returned status ' + res.status);
            }
