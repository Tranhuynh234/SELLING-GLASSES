const opsApp = {
    ...
    state: {
        ...
        inventory: [
            { id: "G-TITAN", name: "Gọng Titanium EyesGlass", stock: 13, unit: "Cái" },
            { id: "L-BLUE", name: "Tròng Blue Control 1.61", stock: 4, unit: "Cặp" },
            { id: "P-BOX", name: "Hộp kính Da EyesGlass", stock: 0, unit: "Cái" }
        ],
        orders: [
            { id: "EG-1100", customer: "Nguyễn Văn Toàn", status: "Chờ lấy hàng", date: "2026-03-19", total: "850.000đ", tracking: "" },
            { id: "EG-1101", customer: "Trương Mỹ Nhân", status: "Đang giao", date: "2026-03-18", total: "1.250.000đ", tracking: "GHN-123456" },
            { id: "EG-1102", customer: "Lê Minh", status: "Hoàn thành", date: "2026-03-15", total: "500.000đ", tracking: "VTP-999888" }
        ],
    ...
    renderTab(tab) {
        ...
        switch(tab) {
            ...
            case 'account': this.renderAccount(area); break;
            default: area.innerHTML = `<div class="card"><h3>Tính năng đang phát triển</h3></div>`;
        }
    },

    // --- 1. TỔNG QUAN (DASHBOARD) ---
    renderDashboard(el) {
        const pendingOrders = this.state.orders.filter(o => o.status === 'Chờ lấy hàng').length;
        const outOfStock = this.state.inventory.filter(i => i.stock <= 0).length;
        const lowStock = this.state.inventory.filter(i => i.stock < 5 && i.stock > 0).length;
    ...
    initDashboardChart() {
        ...
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Đơn hàng', data: [12, 19, 15, 25, 22, 30, 45], borderColor: '#e67e22', tension: 0.4, fill: true, backgroundColor: 'rgba(230, 126, 34, 0.05)'
                }]
            },
    ...
    handleOrderFilter(val) { this.orderFilter = val; this.renderOrders(); },
    ...
    // --- 3. VẬN CHUYỂN ---
    renderShipping(el) {
        const pending = this.state.orders.filter(o => o.status === 'Chờ lấy hàng');
        el.innerHTML = `
            <div class="warehouse-header">
    ...
    renderWarehouse(el) {
        ...
        const filteredData = this.state.inventory.filter(item => {
            ...
            if (this.currentFilter === 'lowstack') return item.stock < 5 && item.stock > 0;
        ...
        target.innerHTML = `
            ...
            <div class="card fade-in"><table class="table-pro">
                ...
                <tbody>${filteredData.map(item => {
                    let stText = item.stock <= 0 ? "Đã hết hàng" : (item.stock < 5 ? "Sắp hết hàng" : "Còn hàng");
                    let stClass = item.stock <= 0 ? "bg-empty" : (item.stock < 5 ? "bg-low" : "bg-ok");
                    let stColor = item.stock <= 0 ? "#c0392b" : (item.stock < 5 ? "#e67e22" : "#27ae60");
    ...
    handleFilter(val) { this.currentFilter = val; this.renderWarehouse(); },
    adjustStock(id, val) { 
        const item = this.state.inventory.find(i => i.id === id);
        if (item) { item.stock = Math.max(0, item.stock + val); this.renderWarehouse(); }
    },
    ...
    handleLogout() {
        if(confirm("Xác nhận đăng xuất khỏi phiên làm việc hiện tại?")) window.location.reload();
    },
    ...
    openUpdateOrderModal(id) {
        ...
        this.setModal(`Cập nhật đơn #${id}`, body, () => {
            ...
            this.closeModal(); this.renderOrders();
        });
    },
    openAddProductModal() {
        ...
        this.setModal("Thêm vật tư mới", body, () => {
            ...
            this.state.inventory.push({ id, name, stock: parseInt(document.getElementById('n-stock').value), unit: document.getElementById('n-unit').value });
            this.closeModal(); this.renderWarehouse();
        });
    }
};
