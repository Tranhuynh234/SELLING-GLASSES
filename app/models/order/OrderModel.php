...
class OrderModel extends BaseModel {
    ...
    // Compatibility methods expected by OrderService
    public function getOrdersForSales($status) {
        // For now, reuse findByStatus which returns orders by status
    ...
    public function getOrderDetailWithCustomer($orderId) {
        ...
        try {
            $sql = "SELECT o.*, c.userId AS customerUserId, c.customerId, c.createdDate as customerCreatedDate
                    FROM {$this->table} o
                    LEFT JOIN customers c ON o.customerId = c.customerId
                    WHERE o.orderId = ? LIMIT 1";
            $order = $this->queryOne($sql, [(int)$orderId]);

            if (!$order) return null;

            $items = $this->queryAll("SELECT oi.*, pv.price, p.name as productName
                FROM order_item oi
                LEFT JOIN product_variant pv ON oi.variantId = pv.variantId
                LEFT JOIN product p ON pv.productId = p.productId
                WHERE oi.orderId = ?", [(int)$orderId]);
    ...
    public function findLatestOrderIdByUserId($userId) {
        ...
        $sql = "SELECT o.orderId FROM {$this->table} o
                JOIN customers c ON o.customerId = c.customerId
                WHERE c.userId = ?
                ORDER BY o.orderDate DESC LIMIT 1";
        $row = $this->queryOne($sql, [(int)$userId]);
        return $row['orderId'] ?? null;
    }

    // Message/chat helper stubs - return default values if the message system isn't implemented yet
    public function markStaffMessagesReadForUser($userId) {
        // Implement if message table exists. For now return true to avoid breaking callers.
        return true;
    }

    public function getMessagesByOrder($orderId) {
        // If there's a messages table implement retrieval. Return empty array as safe default.
        return [];
    }
    ...
    public function saveMessage($orderId, $from, $message) {
        // Save message to message table if available. Return true as default.
        return true;
    }
    ...
    public function getAllCustomerFromOrders() {
        // Return a minimal list of customers who have orders
        $sql = "SELECT DISTINCT c.customerId, c.userId FROM orders o JOIN customers c ON o.customerId = c.customerId";
        return $this->queryAll($sql);
    }
}
